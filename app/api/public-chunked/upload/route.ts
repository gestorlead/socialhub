...
/**
 * Public API: Upload de chunk individual sem autenticação
 * Recebe e salva um chunk específico
 */
export async function POST(request: NextRequest) {
  try {
    console.log('[Public Chunked Upload] Starting public chunk upload')
    
    // Verificar Content-Length
    const contentLength = request.headers.get('content-length')
    console.log('[Public Chunked Upload] Content-Length:', contentLength ? `${(parseInt(contentLength) / 1024).toFixed(1)}KB` : 'não informado')
    ...
    console.log('[Public Chunked Upload] Chunk data:', {
      uploadId,
      chunkIndex,
      totalChunks,
    ...
    try {
      ...
      console.error('[Public Chunked Upload] Failed to read metadata:', error)
      return NextResponse.json({ 
    ...
    console.log('[Public Chunked Upload] Chunk saved:', {
      path: chunkPath,
    ...
    console.log('[Public Chunked Upload] Progress:', {
    ...
    return NextResponse.json({
      ...
      totalChunks,
      progress,
      message: `Public chunk ${chunkIndex} uploaded successfully`,
      isPublicDebug: true
    })

  } catch (error) {
    console.error('[Public Chunked Upload] Error:', error)
    
    return NextResponse.json({
      success: false,
      error: 'Failed to upload public chunk',
...
export async function GET() {
  try {
    console.log('[Public Chunked Upload] GET request - API health check')
    
    return NextResponse.json({
      status: 'ok',
      message: 'Public chunked upload API is working',
      timestamp: new Date().toISOString(),
      isPublicDebug: true
    })
    
  } catch (error) {
    console.error('[Public Chunked Upload] GET error:', error)
    return NextResponse.json({ 
