...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Error fetching Instagram daily stats:', error)
    ...
    const processedStats = stats.map((stat, index) => {
      ...
      const dailyGrowth = prevStat ? {
        ...
        media_growth: stat.media_count - prevStat.media_count,
        impressions_growth: stat.impressions - prevStat.impressions,
        reach_growth: stat.reach - prevStat.reach,
        profile_views_growth: stat.profile_views - prevStat.profile_views
      } : null
    ...
    const summary = stats.length > 0 ? {
      ...
      total_growth: stats.length > 1 ? {
        ...
        media: stats[0].media_count - stats[stats.length - 1].media_count,
        impressions: stats[0].impressions - stats[stats.length - 1].impressions,
        reach: stats[0].reach - stats[stats.length - 1].reach,
        profile_views: stats[0].profile_views - stats[stats.length - 1].profile_views
      } : null,
      averages: {
        ...
        media: Math.round(stats.reduce((sum, s) => sum + s.media_count, 0) / stats.length),
        impressions: Math.round(stats.reduce((sum, s) => sum + s.impressions, 0) / stats.length),
        reach: Math.round(stats.reduce((sum, s) => sum + s.reach, 0) / stats.length),
        profile_views: Math.round(stats.reduce((sum, s) => sum + s.profile_views, 0) / stats.length)
      }
    } : null
    ...
    console.error('Instagram daily stats API error:', error)
    return NextResponse.json({ 
