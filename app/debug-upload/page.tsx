...
export default function DebugUploadPage() {
  ...
  // Upload chunked - dividindo em pedaços
  const testChunkedUpload = async (file: File, chunkSize: number, testIndex: number) => {
    ...
    try {
      ...
      const initResponse = await fetch('/api/debug/init-chunked-upload', {
        method: 'POST',
      ...
      for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
        ...
        const chunkResponse = await fetch('/api/debug/upload-chunk', {
          method: 'POST',
          body: formData
        })
      ...
      const finalizeResponse = await fetch('/api/debug/finalize-chunked-upload', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const runAutomatedTests = async () => {
    ...
    setTests([])
    setLogs([])
    addLog('🚀 Iniciando bateria de testes de upload')

    // Testes com tamanhos progressivos
    const testSizes = [0.1, 0.5, 1, 2, 5, 10, 20, 50] // MB
    const chunkSizes = [100, 400, 1000, 4000] // KB
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">
          🧪 Debug Upload Local - Diagnóstico Completo
        </h1>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-8">
          ...
          <p className="text-sm text-gray-600">
            A bateria de testes criará arquivos de diferentes tamanhos e testará upload direto e chunked.
            Isso nos ajudará a identificar exatamente onde está o limite do servidor.
          </p>
        </div>
